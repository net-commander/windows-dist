...
class XApp_Callback extends XApp_Object
{
	/** @var callable */
	private $cb;
	 ...
	 * @return XApp_Reflection_GlobalFunction|XApp_Reflection_Method
	 */
	public function toReflection()
	{
		if (is_string($this->cb) && strpos($this->cb, '::')) {
			return new XApp_Reflection_Method($this->cb);
		} elseif (is_array($this->cb)) {
			return new XApp_Reflection_Method($this->cb[0], $this->cb[1]);
		} elseif (is_object($this->cb) && !$this->cb instanceof \Closure) {
			return new XApp_Reflection_Method($this->cb, '__invoke');
		} else {
			return new XApp_Reflection_GlobalFunction($this->cb);
		}
	}
