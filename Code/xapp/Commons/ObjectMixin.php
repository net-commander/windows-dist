...
xapp_import('xapp.Commons.*');
/**
 * Object behaviour mixin.
 *
 * @author     David Grudl
 */
class XApp_ObjectMixin
{
	...
	public static function call($_this, $name, $args)
	{
		...
		if ($name === '') {
			throw new XApp_MemberAccessException("Call to class '$class' method without name.");
			...
			if (is_array($_this->$name) || $_this->$name instanceof Traversable) {
				foreach ($_this->$name as $handler) {
					XApp_Utils_Callback::invokeArgs($handler, $args);
				}
			} elseif ($_this->$name !== NULL) {
				throw new XApp_UnexpectedValueException("Property $class::$$name must be array or NULL, " . gettype($_this->$name) ." given.");
			}
			...
			if (count($args) !== ($op === 'get' ? 0 : 1)) {
				...
				throw new XApp_InvalidArgumentException("Argument passed to $class::$name() must be $type, " . gettype($args[0]) . ' given.');
			}
			...
			return XApp_Utils_Callback::invokeArgs($cb, $args);

		} else {
			if (method_exists($class, $name)) { // called parent::$name()
				$class = 'parent';
			}
			throw new XApp_MemberAccessException("Call to undefined method $class::$name().");
		}
	}
	...
	{
		throw new XApp_MemberAccessException("Call to undefined static method $class::$method().");
	}
	...
	public static function set($_this, $name, $value)
	{
		...
		if ($name === '') {
			throw new XApp_MemberAccessException("Cannot write to a class '$class' property without name.");
			...
			throw new XApp_MemberAccessException("Cannot write to $type property $class::\$$name.");
		}
	}
	...
	public static function remove($_this, $name)
	{
		...
		if (!self::hasProperty($class, $name)) { // strict class
			throw new XApp_MemberAccessException("Cannot unset the property $class::\$$name.");
		}
	}
    ...
    /***
     * Merge all into object
     * @param $dstElement
     * @param $newElement
     */
    public static function merge(&$dstElement,$newElement){
        $dstElement=array_merge((array) $dstElement, (array) $newElement);
        return (object)$dstElement;
    }
