 ...
 * @property-read XApp_Security_IIdentity $identity
 * @property-read mixed $id
 * @property-read array $roles
 * @property-read int $logoutReason
 * @property-read XApp_Security_IUserStorage $storage
 * @property   XApp_Security_IAuthenticator $authenticator
 * @property   XApp_Security_IAuthorizator $authorizator
 */
class XApp_Security_User extends XApp_Entity
{
	/** @deprecated */
	const MANUAL = XApp_Security_IUserStorage::MANUAL,
		INACTIVITY = XApp_Security_IUserStorage::INACTIVITY,
		BROWSER_CLOSED = XApp_Security_IUserStorage::BROWSER_CLOSED;
	...
	/** @var XApp_Security_IUserStorage Session storage for current user */
	private $storage;

	/** @var XApp_Security_IAuthenticator */
	...
	/**
	 * @return XApp_Security_IUserStorage
	 */
	...
	{
		$this->logout(TRUE);
		if (!$id instanceof XApp_Security_IIdentity) {
	 ...
	 * @return XApp_Security_IIdentity|NULL
	 */
	 ...
	 * @return self
	 */
	public function setAuthenticator(XApp_Security_IAuthenticator $handler)
	{
	 ...
	 * @return XApp_Security_IAuthenticator
	 */
	public function getAuthenticator($need = TRUE)
	{
		if ($need && !$this->authenticator) {
			throw new Exception('Authenticator has not been set.');
		}
	...
	{
		$flags = ($whenBrowserIsClosed ? XApp_Security_IUserStorage::BROWSER_CLOSED : 0) | ($clearIdentity ? XApp_Security_IUserStorage::CLEAR_IDENTITY : 0);
	 ...
	 * @return bool
	 */
	public function isAllowed($resource = XApp_Security_IAuthorizator::ALL, $privilege = XApp_Security_IAuthorizator::ALL)
	{
	 ...
	 * @return self
	 */
	public function setAuthorizator(XApp_Security_IAuthorizator $handler)
	{
	 ...
	 * @return XApp_Security_IAuthorizator
	 */
	public function getAuthorizator($need = TRUE)
	{
		if ($need && !$this->authorizator) {
			throw new Exception('Authorizator has not been set.');
		}
