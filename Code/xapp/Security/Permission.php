...
xapp_import('xapp.Utils.Callback');
 ...
 */
class XApp_Security_Permission extends XApp_Entity implements XApp_Security_IAuthorizator
{
	...
	private function checkRole($role, $need = TRUE)
	{
		if (!is_string($role) || $role === '') {
			throw new Exception("Role must be a non-empty string.");
	...
	public function isAllowed($role = self::ALL, $resource = self::ALL, $privilege = self::ALL)
	{
		...
		if ($role !== self::ALL) {
			if ($role instanceof XApp_Security_IRole) {
				$role = $role->getRoleId();
			}
			$this->checkRole($role);
		}
		...
		if ($resource !== self::ALL) {
			if ($resource instanceof XApp_Security_IResource) {
