...
class XApp_Json_Util
{
	...
	public static function encode($value, $options = 0)
	{
		...
		set_error_handler(function($severity, $message) { // needed to receive 'recursion detected' error
			...
			throw new XApp_JsonException($message);
		});
		$json = json_encode(
			$value,
		...
		if ($error = json_last_error()) {
			throw new XApp_JsonException(isset(static::$messages[$error]) ? static::$messages[$error] : 'Unknown error', $error);
		}
	...
	public static function decode($json, $options = 0)
	{
		...
		if (!preg_match('##u', $json)) {
			throw new XApp_JsonException('Invalid UTF-8 sequence', 5); // workaround for PHP < 5.3.3 & PECL JSON-C
		}
 ...
 */
class XApp_JsonException extends Exception
{
}
