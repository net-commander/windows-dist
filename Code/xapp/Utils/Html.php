 ...
 * @author     David Grudl
 */
class Html extends XApp_Object implements ArrayAccess, Countable, IteratorAggregate
{
	...
	public function setName($name, $isEmpty = NULL)
	{
		if ($name !== NULL && !is_string($name)) {
			throw new XApp_InvalidArgumentException("Name must be string or NULL, " . gettype($name) ." given.");
		}
	...
	{
		if (is_array($html)) {
			throw new XApp_InvalidArgumentException("Textual content must be a scalar, " . gettype($html) ." given.");
		}
		$this->removeChildren();
	...
	public function insert($index, $child, $replace = FALSE)
	{
		if ($child instanceof Html || is_scalar($child)) {
			...
			throw new XApp_InvalidArgumentException("Child node must be scalar or Html object, " . (is_object($child) ? get_class($child) : gettype($child)) ." given.");
		}
