 ...
 * @author     David Grudl
 */
class Finder extends XApp_Object implements IteratorAggregate
{
	/** @var array */
	private $paths = array();
	...
	{
		if ($this->paths) {
			throw new XApp_InvalidStateException('Directory to search has already been specified.');
		}
		if (!is_array($path)) {
			$path = func_get_args();
		}
	...
	{
		if (!$this->paths) {
			throw new XApp_InvalidStateException('Call in() or from() to specify directory to search.');
	...
	public function size($operator, $size = NULL)
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?((?:\d*\.)?\d+)\s*(K|M|G|)B?\z#i', $operator, $matches)) {
				throw new XApp_InvalidArgumentException('Invalid size predicate format.');
			}
