 ...
 */
class XApp_Http_Session extends XApp_Object
{
	...
	private $options = array(
		...
		// other
			...
			'cache_expire' => 180,   // (default "180")
			'hash_function' => "0",  // (default "0", means MD5)
	 ...
	 * @return string
	 */
	public static function generateRandom($length = 10, $charlist = '0-9a-z')
	{
	...
	public function start()
	{
		...
		//xapp_clog($this->request->getCookie(session_name()));
		...
		try {
			...
			XApp_Callback::invokeSafe('session_start', array(), function ($message) use (& $e) {
				$e = new XApp_InvalidArgumentException($message);
			});
		} catch (\Exception $e) {
		}
		...
		*/
		...
		if (!is_string($browserKey) || !preg_match('#^[0-9a-z]{10}\z#', $browserKey)) {
			$browserKey = self::generateRandom();
		}
	...
	public function regenerateId()
	{
		if (self::$started && !$this->regenerated) {
			if (headers_sent($file, $line)) {
				throw new XApp_InvalidArgumentException('Cannot regenerate session ID after HTTP headers have been sent' . ($file ? " (output started at $file:$line)." : '.'));
			}
			if (session_id() !== '') {
	...
	public function setExpiration($time)
	{
		...
		if (empty($time)) {
			...
			$time = Xapp_DateTime::from($time)->format('U') - time();
			return $this->setOptions(array(
					'gc_maxlifetime' => $time,
    ...
    public static function factory(array $options = null){
         ...
         * Create the session
         */
        $session = new XApp_Http_Session($requestFactory->createHttpRequest(),new XApp_Http_Response());
