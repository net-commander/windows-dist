...
class XApp_Http_SessionSection extends XApp_Object implements \IteratorAggregate, \ArrayAccess
{
	 ...
	 */
	public function __construct(XApp_Http_Session $session, $name)
	{
		if (!is_string($name)) {
			throw new XApp_InvalidArgumentException("Session namespace must be a string, " . gettype($name) ." given.");
		}
