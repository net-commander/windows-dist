...
/***
 * Class XApp_Http_Session
 */
class XApp_Http_Session2
{
	/** Default file lifetime is 3 hours */
	const DEFAULT_FILE_LIFETIME = 1209600;
	...
	/** @var XApp_Http_IRequest */
	private $request;

	/** @var XApp_Http_IResponse */
	private $response;


	public function __construct(XApp_Http_IRequest $request, XApp_Http_IResponse $response)
	{
					...
					foreach ($metadata as $variable => $value) {
						if ((!empty($value['B']) && $browserClosed) || (!empty($value['T']) && $now > $value['T']) // whenBrowserIsClosed || Time
							|| (isset($nf['DATA'][$section][$variable]) && is_object($nf['DATA'][$section][$variable]) && (isset($value['V']) ? $value['V'] : NULL) // Version
								!= XApp_Reflection_ClassType::from($nf['DATA'][$section][$variable])->getAnnotation('serializationVersion')) // intentionally !=
						) {
	...
	{
		if (!self::$started) {
			throw new XApp_InvalidStateException('Session is not started.');
		}
	...
	public function regenerateId()
	{
		if (self::$started && !$this->regenerated) {
			if (headers_sent($file, $line)) {
				throw new XApp_InvalidStateException("Cannot regenerate session ID after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
			}
			if (session_id() !== '') {
	...
	public function setName($name)
	{
		if (!is_string($name) || !preg_match('#[^0-9.][^.]*\z#A', $name)) {
			throw new XApp_InvalidArgumentException('Session name must be a string and cannot contain dot.');
		}
	 ...
	 * @param  string
	 * @param  string
	 * @return XApp_Http_SessionSection
	 * @throws XApp_InvalidArgumentException
	 */
	public function getSection($section, $class = 'XApp_Http_SessionSection')
	{
	 ...
	 * @param  array
	 * @return self
	 * @throws XApp_NotSupportedException
	 * @throws XApp_InvalidStateException
	 */
	...
	private function configure(array $config)
	{
		...
		foreach ($config as $key => $value) {
			...
			if ($value === NULL || ini_get("session.$key") == $value) { // intentionally ==
				...
				if (defined('SID')) {
					throw new XApp_InvalidStateException("Unable to set 'session.$key' to value '$value' when session has been started" . (self::$started ? "." : " by session.auto_start or session_start()."));
				}
				if (isset($special[$key])) {
					...
					throw new XApp_NotSupportedException("Unable to set 'session.$key' to '$value' because function ini_set() is disabled.");
				}
			}
		}
	 ...
	 * @return self
	 */
	public function setStorage(XApp_Http_ISessionStorage $storage)
	{
		if (self::$started) {
			throw new XApp_InvalidStateException('Unable to set storage when session has been started.');
		}
	...
	{
		if (self::$started) {
			throw new XApp_InvalidStateException('Unable to set handler when session has been started.');
		}
	...
	private function sendCookie()
	{
		...
		if($cookie['path']==='/'){
			...
			$cookie['path'] = parse_url($url, PHP_URL_PATH);
		}
		error_log('cookie path : '. '|'. $cookie['path'] . '|');
		*/
		...
		$this->response->setCookie(
			...
			XApp_Http_Response::BROWSER, $cookie['path'], $cookie['domain']
		);
	}
    ...
    public static function factory(array $options = null){
	    ...
	    if(!$options){
		    error_log('have no options');
	    }
        /***
         * Create Request
         */
        $requestFactory = new XApp_Http_RequestFactory();
         ...
         */
        $response  = new XApp_Http_Response();
        /***
         * Create the session
         */
        $session = new XApp_Http_Session($request,$response);
