 ...
 */
xapp_import('xapp.Security.IUserStorage');
/**
 ...
 * @author  Jan Tichý
 */
class XApp_Http_UserStorage implements XApp_Security_IUserStorage
{
	/** @var string */
	private $namespace = '';
	...
	public function  __construct(XApp_Http_Session $sessionHandler)
	{
	 ...
	 * @return self
	 */
	public function setIdentity(XApp_Security_IIdentity $identity = NULL)
	{
	...
	protected function getSessionSection($need)
	{
		...
		$this->sessionSection = $section = $this->sessionHandler->getSection('XApp.Http.UserStorage/' . $this->namespace);

		if (!$section->identity instanceof XApp_Security_IIdentity || !is_bool($section->authenticated)) {
			$section->remove();
		}
