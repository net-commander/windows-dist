 ...
 */
class XApp_Http_RequestFactory
{
	/** @internal */
	...
	public function createHttpRequest()
	{
		...
		$requestUrl = XApp_Utils_Strings::replace($requestUrl, $this->urlFilters['url']);
		$tmp = explode('?', $requestUrl, 2);
		$path = XApp_Http_Url::unescape($tmp[0], '%/?#');
		$path = XApp_Utils_Strings::fixEncoding(XApp_Utils_Strings::replace($path, $this->urlFilters['path']));
		$url->setPath($path);
		...
		if (get_magic_quotes_gpc()) {
			$post = XApp_Http_Helpers::stripslashes($post, $useFilter);
			$cookies = XApp_Http_Helpers::stripslashes($cookies, $useFilter);
		}
		...
		*/
		...
		return new XApp_Http_Request($url, NULL, $post, $files, $cookies, $headers, $method, $remoteAddr, $remoteHost, $rawBodyCallback);
	}
