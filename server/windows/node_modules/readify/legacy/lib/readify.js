...
var sortFiles = currify(_sortFiles);
...
var negative = function negative(fn) {
    return function () {
        return -1 * fn.apply(undefined, arguments);
    };
};
...
var compare = function compare(attr, order) {
...
function readify(path, options, fn) {
    ...
    readdir(path).then(sortFiles(sort, order)).then(parseAllStats(type)).then(fillJSON(path, type)).then(good(fn)).catch(fn);
}
...
function _replaceProperty(obj, prop, item) {
    ...
    return assign(item, _defineProperty({}, prop, data));
}

function _sortFiles(attr, order, array) {
    var isFile = not(isDir);
    var dirs = array.filter(isDir).sort(compare('name'));
